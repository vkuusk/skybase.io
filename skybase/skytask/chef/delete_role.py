...
def chef_delete_role_add_arguments(parser):
    """
    ...
    """
    chef_delete_role = parser.add_argument_group('chef delete-role')
    chef_delete_role.add_argument(
        '-r',
        '--role',
        dest='role',
        action='store',
...
class DeleteRole(SkyTask):
    ...
    def preflight_check(self):
        ...
        try:
            self.role = sky_chef_actions.get_role(self.planet, self.args.get('role'), self.logger)
        except Exception as e:
            self.preflight_check_result.status = 'FAIL'
            preflight_result.append(skybase.exceptions.SkyBaseValidationError('could not find role "{0}": {1}'.format(self.args.get('role'), simple_error_format(e))))
    ...
    def execute(self):
        # delete role
        try:
            sky_chef_actions.delete_role(self.planet, self.args.get('role'), self.logger)
        except Exception as e:
            self.result.status = sky_cfg.API_STATUS_FAIL
            raise skybase.exceptions.SkyBaseResponseError('Could not delete role "{0}" from Chef: {1}'.format(self.args.get('role'), simple_error_format(e)))

        self.result.status = sky_cfg.API_STATUS_SUCCESS
        self.result.output = 'Role "{0}" has been deleted from Chef'.format(self.args.get('role'))
